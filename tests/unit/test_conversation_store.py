...
async def test_append_then_get_roundtrip() -> None:
    ...
    await store.append(
        TENANT_ID, "c1", [user("hi"), assistant("hello")], items("hi", "hello"), 0, usage_tokens=0
    )
...
async def test_append_extends_existing_history_in_order() -> None:
    store = InMemoryConversationStore()
    await store.append(
        TENANT_ID, "c1", [user("one"), assistant("1")], items("one"), 0, usage_tokens=0
    )

    await store.append(
        TENANT_ID, "c1", [user("two"), assistant("2")], items("two"), 1, usage_tokens=0
    )
...
async def test_stale_revision_is_rejected_and_commits_nothing() -> None:
    ...
    with pytest.raises(ConversationConflictError):
        await store.append(
            TENANT_ID, "c1", [user("two"), assistant("2")], items("two"), 0, usage_tokens=0
        )
...
async def test_get_is_scoped_to_the_tenant_that_created_the_conversation() -> None:
    store = InMemoryConversationStore()
    await store.append(
        TENANT_ID, "c1", [user("hi"), assistant("hello")], items("hi"), 0, usage_tokens=0
    )
...
async def test_append_does_not_cross_tenant_boundaries() -> None:
    store = InMemoryConversationStore()
    await store.append(TENANT_ID, "c1", [user("t1-turn")], items("t1-turn"), 0, usage_tokens=0)
    ...
    await store.append("t2", "c1", [user("t2-turn")], items("t2-turn"), 0, usage_tokens=0)
...
async def test_failed_append_is_all_or_nothing() -> None:
    ...
    with pytest.raises(RuntimeError):
        await store.append(
            TENANT_ID, "c1", [user("hello"), assistant("hi")], poisoned, 0, usage_tokens=0
        )
...
async def test_messages_are_frozen_so_aliases_cannot_rewrite_history() -> None:
    ...
    appended = user("hi")
    await store.append(
        TENANT_ID, "c1", [appended, assistant("hello")], items("hi"), 0, usage_tokens=0
    )
...
async def test_appended_replay_items_are_copied_not_aliased() -> None:
    ...
    await store.append(
        TENANT_ID, "c1", [user("hi"), assistant("hello")], caller_items, 0, usage_tokens=0
    )
