...
| Purge protection | **off** (omitted + read-back asserted) | Enabling it is irreversible and blocks early purge. Production wants it on; an ephemeral lab that promises "every create script has a teardown" cannot have it. One API quirk, hit live 2026-08-06: the property **cannot be passed explicitly as `false`** — the create call rejects it ("cannot be set to false … irreversible action"); it is either omitted or `true`. So "off" is expressed the only way the API allows: the script never passes the flag and fails if the created vault reads back purge-protected. |
