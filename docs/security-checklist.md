...
- [ ] Every script that creates a probe or demo Azure resource pairs with a delete script — OpenAI, Search, Key Vault, Content Safety and the Entra app registration (`infra/scripts/create-*.sh` / `delete-*.sh`), plus APIM (`configure-apim.sh` / `delete-apim.sh` — the create side is named `configure-apim.sh` and does not match the `create-*.sh` glob), and `teardown.sh` for the whole resource group. Two named exceptions: `create-budget-alert.sh` has no delete counterpart because the subscription budget alert is deliberately persistent, not ephemeral; `deploy-container-app.sh` has no dedicated delete script and is removed only via `teardown.sh`
