...
- [ ] Every script that creates a probe or demo Azure resource pairs with a delete script — OpenAI, Search, Key Vault, Content Safety and the Entra app registration (`infra/scripts/create-*.sh` / `delete-*.sh`), plus APIM (`configure-apim.sh` / `delete-apim.sh` — the create side is named `configure-apim.sh` and does not match the `create-*.sh` glob), the container registry (`create-acr.sh` / `delete-acr.sh`), the Container Apps deployment (`deploy-container-app.sh` / `delete-container-app.sh` — which also deletes the managed identity's role assignments, and proves them gone, before the identity itself, [container-apps.md](container-apps.md#9-teardown-ordering-is-a-contract-not-a-convenience)), and `teardown.sh` for the whole resource group. One named exception: `create-budget-alert.sh` has no delete counterpart because the subscription budget alert is deliberately persistent, not ephemeral
