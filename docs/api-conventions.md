...
Cost is metered, not estimated: every turn reports the provider-reported token counts (a request-level usage signal for attribution and guardrails — not a billing record; the invoice and Cost Management meters remain the source of truth), and a per-conversation budget is enforced before inference.
