...
| `update-container-app.sh` | Point an already-deployed app at a new image by digest and refuse success unless the template reports it, no known failure state appears, and `/health` returns the exact body — it cannot establish which revision served that response: pre-mutation template snapshot (verbatim, tag or digest, a rollback candidate only), `az containerapp update --image`, fail-closed template read-back, revision-state failure detector, `/health` exact-body smoke — no automatic rollback, only an advisory command printed on failure | working |
