...
| `AZ_CONTENT_SAFETY_NAME` | all three | **`run-content-safety-probe.sh`: a name prefix** (≤ 57 chars) that the run turns into a unique account name; the two child scripts take it as the final, globally unique name — also used as the custom subdomain |
